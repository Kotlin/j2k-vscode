...
function flatText(n: any): string {
  ...
  if (Array.isArray(n)) {
    return n.map(flatText).join("");
  }
  if (typeof n === "object") {
    if ("image" in n && typeof n.image === "string") {
      return n.image;
    }
    if ("children" in n && n.children) {
      return Object.values(n.children).map(flatText).join("");
    }
...
export function extractAddresses(javaCode: string): string[] {
  ...
  function pushTypeName(n: CstNode): void {
    ...
    const ident =
      child<any>(typeId, "Identifier") ?? child<any>(n as any, "Identifier");
